# Health Care

> Health care is easier when Jasper knows where to go before feeling terrible.

## If You Are in Immediate Danger

**Call or text 911.**

If on campus, also contact [Campus Safety](https://www.lclark.edu/about/campus_safety/) at **503-768-7777** when appropriate.

## Where to Go

| Situation | Typical Option |
| --- | --- |
| Routine issue | Primary care / student health |
| Same-day non-emergency | Urgent care |
| Serious or life-threatening | ER / 911 |

## Jasper's Health Care Setup

> Start here for normal doctor visits, medication questions, annual care, referrals, and non-emergency health problems.

### Primary Care

| Item | Details |
| --- | --- |
| Clinic | [Providence Primary Care - Mary's Woods](https://www.providence.org/locations/or/primary-care-marys-woods) |
| Address | 17550 Provost St, Suite 201, Lake Oswego, OR 97034 |
| Phone | 503-872-2440 |
| Fax | 503-513-3355 |
| Hours | Monday-Friday, 8 a.m.-5 p.m. |
| Insurance note | Providence lists **TRICARE Select** as accepted, but Jasper should still confirm coverage before the first visit. |

### Insurance

Jasper will be using **TRICARE Select** through **TriWest Healthcare Alliance** in the TRICARE West Region.

Important basics:

- [TRICARE Select](https://www.tricare.mil/About/Changes/Plans/Select?p=1) is a self-managed preferred provider plan.
- [TriWest](https://tricare.triwest.com/) administers TRICARE in the West Region, which includes Oregon.
- TRICARE Select usually does **not** require referrals for most primary or specialty appointments.
- Some services may still require **pre-authorization**.
- To pay less, use TRICARE-authorized **network providers** when possible.
- Jasper's **Uniformed Services ID card** is the proof of TRICARE coverage.

When in doubt, call TriWest before the appointment:

- **TriWest / TRICARE West:** 888-TRIWEST (**888-874-9378**)

### Before the First Providence Visit

- [ ] Call Providence and confirm they still accept Jasper's exact TRICARE Select coverage.
- [ ] Ask whether the provider is in-network with TriWest/TRICARE West.
- [ ] Ask whether new patient forms can be completed before the visit.
- [ ] Bring Uniformed Services ID, photo ID, medication list, allergies, and immunization records.
- [ ] Ask how to set up Providence MyChart.
- [ ] Ask where prescriptions should be sent.
- [ ] Arrive early the first time.

### Getting There by Transit

Best starting point:

- [Google Maps transit directions from Lewis & Clark to Providence Primary Care - Mary's Woods](https://www.google.com/maps/dir/?api=1&origin=Lewis%20%26%20Clark%20College%2C%20615%20S%20Palatine%20Hill%20Rd%2C%20Portland%2C%20OR%2097219&destination=Providence%20Primary%20Care%20-%20Mary%27s%20Woods%2C%2017550%20Provost%20St%20Suite%20201%2C%20Lake%20Oswego%2C%20OR%2097034&travelmode=transit)

Likely transit pattern:

- Use [TriMet Line 35 - Macadam/Greeley](https://trimet.org/schedules/r035.htm).
- From the L&C area, board Line 35 toward **Oregon City Transit Center**.
- Get off near the Marylhurst / Mary's Woods area, then walk to **17550 Provost St**.
- For the return trip, use Line 35 toward **Portland City Center / University of Portland**.

Before leaving:

- [ ] Check the route in [TriMet Trip Planner](https://trimet.org/home/planner/) or Google Maps.
- [ ] Check the return route.
- [ ] Check service alerts.
- [ ] Leave extra time for the walk from the bus stop.
- [ ] Bring rain gear.
- [ ] Save the clinic phone number in case the bus is delayed.

> Transit routes and stop choices can change. Always verify the route on the day of the appointment.

## Lewis & Clark Resources

- [Student Health Center](https://www.lclark.edu/offices/health_center/) — On-campus physical and mental health care for registered students.
- [Student Counseling Center](https://www.lclark.edu/offices/counseling/) — Counseling, crisis counseling, groups, and referrals.
- [Student Support Resources](https://college.lclark.edu/student_life/student_support_resources/) — Where to start when a student needs help.
- [LGBTQIA+ Resources](https://college.lclark.edu/student_life/wellness/lgbtqia-resources/) — Queer and trans health and campus support links.

Verify current hours, fees, and appointment rules before relying on them.

The L&C Student Health Center can still be useful for convenient campus care. Because Jasper has TRICARE Select, he should ask about fees, insurance, and whether it makes more sense to use Providence for ongoing care.

## Insurance Basics

Keep a photo of your insurance card.

Useful terms:

- **In-network:** providers your plan usually covers better.
- **Out-of-network:** may cost more.
- **Copay:** fixed amount due for a visit or prescription.
- **Deductible:** amount you may pay before insurance covers more.
- **Referral:** approval or recommendation needed for some care.
- **Pharmacy:** where prescriptions are filled.

## Prescriptions

- [ ] Know medication names and doses.
- [ ] Refill before running out.
- [ ] Ask how to transfer a prescription to a Portland pharmacy.
- [ ] Keep prescriber contact information.
- [ ] Know what to do if a refill is delayed.

For ongoing medications, ask Providence:

- which pharmacy to use,
- how refill requests work,
- how much notice they need,
- what to do if medication is lost, delayed, or runs out.

## Missing Class Because of Illness

Email professors early.

Say:

- that you are sick,
- what class or assignment is affected,
- what you are asking for,
- when you expect to follow up.

Check school policies when illness affects multiple classes.

## Mental Health

If it is urgent or after hours, use crisis support:

- **988** for the Suicide and Crisis Lifeline.
- Campus Safety at **503-768-7777** for on-campus immediate safety concerns.
- 911 for off-campus emergencies.

For non-emergency support, contact the Student Counseling Center.

## Medical Info to Keep Handy

- [ ] Insurance card.
- [ ] Medication list.
- [ ] Allergies.
- [ ] Emergency contacts.
- [ ] Doctor or prescriber contact information.

## Related Guides

- [Oh Shit Guide](oh-shit-guide.md)
- [Community and Support](../college-life/community-and-support.md)
- [College Life 101](../academic/college-life-101.md)
